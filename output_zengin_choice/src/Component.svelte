  ...
  */
    ...
    for (const row of dataArray2) {
      ...
      try {
        const result = await API.saveRow({
          // TODO: このテーブルIDはハードコードされています。コンポーネントのプロパティとして外から渡すことを検討してください。
          tableId: "ta_05a0b492529f4ae18e9c4999a0809bc6",
          _id: targetId,
          Status: "05振込処理",
          UpdatedBy: exUser
        });
  ...
  function toCSV(dataArray, dataArray2) {
    ...
    const detailLines = rowsDataCategory2
      .map(r => {
        countDataCategory2++;
        totalTransferAmount += r.EarlyPaymentAmount;

        // AccountNumber を7桁ゼロ埋め
        const paddedAccountNumber = String(r.AccountNumber).padStart(7, "0");
        const accountTypeCode = ({ "普通": 1, "当座": 2 }[r.AccountType] ?? "");
  ...
  function downloadCsv() {
    // 確認ダイアログ
    if (!confirm("全銀データを出力し、ステータスを振込処理にしますか？")) {
